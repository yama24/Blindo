...
class Administrator extends CI_Controller
{
	...
	{
		$data['title'] = 'Vendor - Admin Panel';
		$data['vendor'] = $this->db->get('vendor');
		$this->load->view('templates/header_admin', $data);
		$this->load->view('administrator/vendor', $data);
		$this->load->view('templates/footer_admin');
	}
	...
	public function sending_order($id)
	{
		...
		$this->email->from($this->config->item('mail_account'), $this->config->item('app_name'));
		$this->email->to($buyer['email']);
		$this->email->subject('Pesanan Telah Dikirim ' . $id);
		$this->email->message($mesg);
		$this->email->send();
		$this->session->set_flashdata('upload', "<script>
            swal({
	...
	public function add_banner_setting_post()
	{
		...
		if ($upload['result'] == 'success') {
			...
			if ($insert) {
				...
				$this->session->set_flashdata('failed', "<div class='alert alert-danger' role='alert'>
                Gagal menambah banner, gambar yang kamu upload tidak berukuran 1770x702px.
                </div>");
